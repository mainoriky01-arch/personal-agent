...
// Deterministic fake LLM (unused by the seatbelt path, required by the ctor).
...
describe("Seatbelt state mirror (§1.7 device loop) — Api methods", () => {
  ...
  it("trigger marks the user active with the target app and a since timestamp", () => {
    ...
    // The status getter reflects the same live state.
