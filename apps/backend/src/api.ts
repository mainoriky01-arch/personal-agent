 ...
 * The native client applies the shield on-device and reports the barrage as
 * active via `POST /seatbelt/trigger`, then clears it via `POST /seatbelt/resolved`.
 * The backend only mirrors this state (the device is the enforcer), so it is
 * transient in-process state, not a durable record.
 */
...
export class Api {
  ...
  triggerSeatbelt(input: { userId?: unknown; targetApp?: unknown }): ApiResult<SeatbeltStatus> {
    ...
    const status: SeatbeltStatus = { userId, active: true, targetApp, since: this.clock.nowIso() };
    // Re-triggering an already-active user overwrites the record (latest wins),
    // keeping the endpoint idempotent from the device's perspective.
   ...
   * Exposed for tests and introspection; the device drives state via the routes. */
